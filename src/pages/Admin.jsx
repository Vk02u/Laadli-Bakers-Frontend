...
export default function Admin() {
    ...
    async function load() {
      try {
        ...
        const [cakesRes, ordersRes, reviewsRes] = await Promise.all([
          fetch('https://laadli-bakers-backend-production.up.railway.app/api/cakes'),
          fetch('https://laadli-bakers-backend-production.up.railway.app/api/orders', { headers }),
  ...
  async function addCake(e) {
    ...
    const res = await fetch('https://laadli-bakers-backend-production.up.railway.app/api/cakes', {
      method: 'POST',
  ...
  async function saveEdit(e) {
    ...
    const res = await fetch(`https://laadli-bakers-backend-production.up.railway.app/api/cakes/${editingId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    })
  ...
  async function removeCake(id) {
    ...
    const res = await fetch(`https://laadli-bakers-backend-production.up.railway.app/api/cakes/${id}`, { method: 'DELETE' })
  ...
  async function toggleOrderStatus(order) {
    ...
    console.log(`Updating order ${order.id} to status: ${next}`)

    try {
    const res = await fetch(
        `https://laadli-bakers-backend-production.up.railway.app/api/orders/${order.id}/status`,
        {
          method: 'PUT',
          headers: {
    ...
    if (res.ok) {
      ...
      if (selectedOrder && selectedOrder.id === order.id) setSelectedOrder(updated)
        alert(`Order ${order.id} marked as ${next}!`)
      } else {
        const errorText = await res.text()
        console.error('Failed to update order status:', errorText)
        alert(`Failed to update order status: ${res.status} - ${errorText}`)
      }
    } catch (error) {
  ...
  async function deleteOrder(orderId) {
    ...
    const res = await fetch(`https://laadli-bakers-backend-production.up.railway.app/api/orders/${orderId}`, {
      method: 'DELETE',
  ...
  async function deleteReview(reviewId) {
    ...
    const res = await fetch(`https://laadli-bakers-backend-production.up.railway.app/api/reviews/${reviewId}`, {
      method: 'DELETE',
      headers: getAuthHeaders()
    })
  ...
  async function togglePinReview(reviewId) {
    const res = await fetch(`https://laadli-bakers-backend-production.up.railway.app/api/reviews/${reviewId}/pin`, {
      method: 'PATCH',
      headers: getAuthHeaders()
    })
    if (res.ok) {
